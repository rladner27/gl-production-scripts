...
# Load IT-ToolBox Module
try {
    Import-Module IT-ToolBox -ErrorAction Stop
} catch {
    Write-Warning "Unable to load IT-ToolBox PowerShell Module"
}

# Define Credentials
[string]$userName = 'ENTER EMAIL HERE'
[SecureString]$securePwd = ConvertTo-SecureString -String (New-StringDecryption -EncryptedString 'ENTER ENCRYPTED PASSWORD HERE') -AsPlainText -Force

# Create the credential object
[pscredential]$credential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $userName, $securePwd
...
foreach ($user in $notifyUsers) {
    ...
    <a href=""mailto:service@greenloopsolutions.com""?Subject=Password Expiry Assistance"">service@greenloopsolutions.com</a> | 1-888-877-4395
    </font>"
    ...
    # Set up Send-Mailmessage parameters
    $params1 = @{
    ...
    try {
        ...
        if ($interval) {
            ...
            if (($interval) -Contains ($daysToExpire)) {
                ...
                #Write-Output "Command: Send-Mailmessage -smtpServer $smtpServer -port 2525 -from $from -to $emailaddress -subject $subject -body $body -bodyasHTML -priority High -Encoding $textEncoding -ErrorAction Stop"
                Send-Mailmessage @params1 -UseSsl -bodyasHTML -ErrorAction Stop
...
if ($logging) {
    ...
    if ($reportTo) {
        ...
        try {
            ...
            Send-Mailmessage @params2 -UseSsl -bodyasHTML -ErrorAction Stop
        } catch {
            $errorMessage = $_.Exception.Message
